...
```bash
cmake -S Src -B build -DCMAKE_BUILD_TYPE=Release -DCMAKE_EXPORT_COMPILE_COMMANNDS=1 -DTREE_2D_DUMP=1 -DTREE_3D_DUMP=1  -DUSE_LOGGER=1 -DVERBOSE_OUTPUT=1
```
...
`TREE_2D_DUMP` - включает в проект функацию графического дампаоктодерева в формат `png`. Подробнее будет описано далее.
...
verbose      # для включение подробного вывода -DVERBOSE_OUTPUT=1
2d-dump      # для подключения функции графического 2D дампа дерева -DTREE_2D_DUMP
3d-dump      # для подключения функции графического 3D дампа дерева -DTREE_3D_DUMP
```
Пример использования:
```bash
bash start.sh use-logger 3d-dump 
```
- сгенерирует систему сборки и соберет проект c опциями `-DUSE_LOGGER=1` и `-DTREE_3D_DUMP`
...
Так же предоставлен скрипт `debug_start.sh`, который подключит все возможные опции для дебага и соберет проект вместе с ними.
...
```
Класс треугольника является шаблонным, и принимает тип координат вершин треугольника, который обяза быть floating-point типом:
```cpp
...
Первый оперирут типом `float`, второй - `long double`ю
...
![граввиз_зарешал](./assets/2d-dump.png)
