...
class octree_t
{
    ...
    private:
        ...
        std::string create_dot_out_dir          (const std::string& dot_out_dir)                              const;
        std::string create_img_dir              (const std::string& img_dir)                                  const;
        std::string get_png_file_path           (std::string_view img_name, const std::string& img_dir)       const;
        std::string get_dot_file_path           (                           const std::string& dot_out_dir)   const;
...
octree_t<coordinate_t>::dont_need_to_split_node(const node_t* node) const
{
    ...
    return
        (node->bbox.get_side_length() <= min_bbox_side_length_) ||
...
octree_t<coordinate_t>::find_intersections_helper(const node_t* node)
{
    ...
    for (size_t child_it = 0; child_it < children_quant; ++child_it)
    {
        ...
        if (!child_ptr /*|| child_ptr->is_leaf*/) continue;
...
octree_t<coordinate_t>::parse_result_of_triagles_intersection(size_t triangle_1_index, size_t triangle_2_index)
{
    ...
    const bool is_both_good = numbers_of_triangles_that_have_intersections_with_others_[triangle_1_index] &&
...
{
    bool is_points_valid = lbp.is_valid() &&
                           rtp.is_valid();
    if (!is_points_valid)
        return false;
    ...
    bool is_square = (Math::Compare::compare(len_x, len_y)) &&
                     (Math::Compare::compare(len_x, len_z)) && // len_x = len_y = len_z >= 0
                     (len_x >= 0);
...
octree_t<coordinate_t>::bounding_box_t::is_intersect(const bounding_box_t& box) const
{
    ...
    return
            lbp.is_between_2_points(box.lbp, box.rtp) ||
            rtp.is_between_2_points(box.lbp, box.rtp) ||
        box.lbp.is_between_2_points(    lbp,     rtp) ||
...
octree_t<coordinate_t>::dump_2d(std::string_view img_name) const
{
    ...
    create_dot_out_dir(dot_out_dir);
    create_img_dir    (img_dir    );
 
    std::string img_file = get_png_file_path(img_name, img_dir);
    std::string dot_file = get_dot_file_path(dot_out_dir);
...
template <typename coordinate_t>
std::string
octree_t<coordinate_t>::create_dot_out_dir(const std::string& dot_out_dir) const
{
    static bool dot_dir_already_made = false;
    
    if (dot_dir_already_made)
        return dot_out_dir;

    std::ostringstream create_dot_dir_command;
    create_dot_dir_command << "mkdir -p " << dot_out_dir;

    int sys_result = std::system(create_dot_dir_command.str().c_str());

    if (sys_result == EXIT_FAILURE)
        return "";

    dot_dir_already_made = true;
...
template <typename coordinate_t>
std::string
octree_t<coordinate_t>::create_img_dir(const std::string& img_dir) const
{
    static bool img_dir_already_made = false;

    if (img_dir_already_made)
        return img_dir;

    std::ostringstream create_img_dir_command;
    create_img_dir_command << "mkdir -p " << img_dir;

    int sys_result = std::system(create_img_dir_command.str().c_str());

    if (sys_result == EXIT_FAILURE)
        return "";

    img_dir_already_made = true;
...
template <typename coordinate_t>
std::string
octree_t<coordinate_t>::get_dot_file_path(const std::string& dot_out_dir) const
{
    return dot_out_dir + ".dot";
}
...
octree_t<coordinate_t>::create_image_from_dot(std::string_view dot_file, std::string_view img_file) const
{
    ...
    std::system(dot_command.str().c_str());
}
